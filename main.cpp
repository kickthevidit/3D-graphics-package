...
public:
  ...
  virtual bool RayIntersect(const Vec3 &orig, const Vec3 &dir,
                            double &t0) const {
    return false;
  } // check if a ray intersects with our shape
...
public:
  ...
  Sphere(const Vec3 &c, double r, Pixel &p, double d, Vec3 al)
      : center(c), radius(r), Shape(p, d, al) {
    if (radius < 0)
      radius *= -1;
  }
...
class Checkerboard : public Shape {
  ...
  Vec3 side; // dir vec showing board orientation, going from one corner to
             // another
  Vec3 centre;

public:
  Checkerboard(Vec3 n, double d_, Pixel p, Vec3 cen, Vec3 s, Material &m)
      : normal(n), d(d_), color_2(p), Shape(m), centre(cen), side(s) {}
...
bool EnvironmentIntersect(const Vec3 &orig, const Vec3 &dir,
  ...
  // 		point = pt;
  // 		point_n = Vec3(0,1,0);
  // 		obj_mat.diffuse_color = (int(.5 * point.I + 1000) + int(.5 *
  // point.K)) & 1 ? Vec3(1, 1, 1) : Vec3(1, .7, .3);
...
Pixel CastRay(const Vec3 &orig, const Vec3 &dir, const vector<Shape *> &objects,
  ...
  for (const Light &l : lights) {
    ...
    diffuse_light_intensity +=
        l.intensity * max((double)0, light_dir * coord_n);
    // if (diffuse_light_intensity == 0) std::cout << l.intensity << ',' <<
    // light_dir * coord_n << '\n';
    specular_light_intensity +=
        pow(max((double)0, Reflect(light_dir, coord_n) * dir),
            mat.specular_exponent) *
        l.intensity;
  }

  // std::cout << "Final Color: " << (mat.diffuse_color *
  // diffuse_light_intensity * mat.albedo.I + Pixel(Vec3(1,1,1)) *
  // specular_light_intensity * mat.albedo.J + reflect_color * mat.albedo.K) <<
  // '\n';
...
diffuse_light_intensity * mat.albedo.I + Pixel(Vec3(1,1,1)) *
specular_light_intensity * mat.albedo.J + reflect_color * mat.albedo.K) << '\t';
          std::cout << mat.diffuse_color << ',' << diffuse_light_intensity <<
',' << mat.albedo.I << '\t'; std::cout << Pixel(Vec3(1,1,1)) *
specular_light_intensity << ',' << mat.albedo.J << '\t'; std::cout <<
reflect_color << ',' << mat.albedo.K << '\n';
} */
...
void WriteGif(vector<vector<vector<Pixel>>> &pixels, string file_path) {

  if (file_path.substr(file_path.size() - 4, 4) != ".gif")
    file_path += ".gif";

  unsigned frames = pixels.size();
  unsigned width = pixels.at(0).at(0).size();
  unsigned height = pixels.at(0).size();
  int delay = 100;

  std::cout << "frames,width,height: " << frames << ',' << width << ','
            << height << '\n';

  GifWriter g;
  GifBegin(&g, file_path.c_str(), width, height, delay);
  for (vector<vector<Pixel>> &p : pixels) {
    vector<uint8_t> data(width * height * 4, 0);
    GifWriteFrame(&g, PixelsToData(p).data(), width, height, delay);
  }
  GifEnd(&g);
}
} // namespace Render
...
int main() {
  ...
  std::cout << "writing gif\n";
  Render::WriteGif(p, "test/sample-gif");
